...
Using the trimmed and quality checked reads sequences in FASTQ format as input data, the next step is metagenome assembly using Megahit for DNA assembly. Megahit is a software used for more complex NGS reads.  The purpose of this step is to merge overlapping sequences into a longer sequence instead. The output data is assembled genomic sequences. 
...
The software eggNOGmapper is used in online Uppmax for functional annotation. The input is the bins with the best quality and the output is functional gene annotations. The software is for functional annotations of already predicted sequences. 

Step 7-  Abundance of different organisms/bins.  
This method helps me understand how common each organism is in each bin. The input will be the 
